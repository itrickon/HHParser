...
class HHParse:
    ...
    async def extract_city_from_location(self, location_text: str) -> str:
        ...
        remove_words = ["район", "область", "край", "республика", "г.", "м.", "ул.", "пр.", "д."]
    ...
    async def parse_vacancy_card(self, vacancy_card) -> dict:
        ...
        try:
            ...
            # Название вакансии
            try:
                title_el = await vacancy_card.query_selector('[data-qa="serp-item__title"]')
                if title_el:
            ...
            # Компания
            try:
                company_el = await vacancy_card.query_selector('[data-qa="vacancy-serp__vacancy-employer"]')
                if company_el:
            ...
            # Город
            try:
                city_el = await vacancy_card.query_selector('[data-qa="vacancy-serp__vacancy-address"]')
                if city_el:
    ...
    async def parse_vacancy_page(self, vacancy_url: str) -> dict:
        ...
        try:
            ...
            await vacancy_page.goto(vacancy_url, wait_until="domcontentloaded", timeout=self.NAV_TIMEOUT)
            await self.human_sleep(*self.POST_NAV_IDLE)
            await self.human_scroll_jitter(vacancy_page)
            
            # Название вакансии
            try:
                title_el = await vacancy_page.query_selector('[data-qa="vacancy-title"]')
                if title_el:
                    page_data["vacancy"] = (await title_el.text_content()).strip()
            except:
                pass
            ...
            # Компания
            try:
                company_el = await vacancy_page.query_selector('[data-qa="vacancy-company-name"]')
                if company_el:
                    company_text = await company_el.text_content()
                    page_data["company"] = ' '.join(company_text.split()).strip()
            except:
            ...
            # Город
            try:
                city_el = await vacancy_page.query_selector('[data-qa="vacancy-view-location"]')
                if city_el:
                    city_text = await city_el.text_content()
                    page_data["city"] = await self.extract_city_from_location(city_text)
                else:
            ...
            try:
                if await self.click_contact_button(vacancy_page):
                    page_data["phone"] = await self.extract_phone_from_contact_popup(vacancy_page)
    ...
    async def get_vacancy_cards(self) -> List:
        ...
        try:
            ...
            vacancy_cards = await self.page.query_selector_all('[data-qa="vacancy-serp__vacancy"]')
            print(f"Найдено карточек вакансий: {len(vacancy_cards)}")
            return vacancy_cards[:min(20, self.max_num_firm - len(self.list_of_companies))]
        except Exception as e:
    ...
    async def parse_main(self, update_callback=None):
        ...
        async with async_playwright() as playwright:
            try:
                ...
                await self.page.goto(self.link, wait_until="domcontentloaded", timeout=self.NAV_TIMEOUT)
