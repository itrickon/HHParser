...
class HHParser(ttk.Frame):
    ...
    def start_sorting(self):
        if not self.phone_excel_path:
            ...
            messagebox.showerror("Ошибка", "Сначала выберите Excel файл")
            return
        
        if not os.path.exists(self.phone_excel_path):
            self.log_message(f"Ошибка! Файл не найден: {self.phone_excel_path}")
            messagebox.showerror("Ошибка", f"Файл не найден: {os.path.basename(self.phone_excel_path)}")
            return
    ...
    def create_phone_params(self):
        ...
        self.excel_file_btn = ttk.Button(self.phone_frame, text="Excel файл", 
                                        command=self.btn_open, width=20)
        ...
        ttk.Label(self.phone_frame, textvariable=self.excel_file_path, 
                foreground="gray", wraplength=300).grid(row=1, column=1, padx=5, pady=0, sticky=tk.W)
    ...
    def btn_open(self):
        ...
        if self.file_path:
            ...
            try:
                ...
                self.log_message(f"Теперь можете запустить парсинг по mail.")
    ...
    def btn_about(self):
        ...
        text_widget = Text(frame, wrap=tk.WORD, width=55, height=15, 
                        font=("Arial", 10))
        ...
        about_text = [
        "       HHParser\n\n",
        "  Данный инструмент предназначен для сортировки данных.\n\n",
        "    Версия 0.0.4\n\n",
        "  Возможности:\n",
